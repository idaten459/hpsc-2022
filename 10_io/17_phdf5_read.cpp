...
int main (int argc, char** argv) {
  ...
  hsize_t Nglobal[2] = {NX/dim[0], NY/dim[1]};
  ...
  hid_t localspace = H5Screate_simple(2, Nglobal, NULL);
  H5Sselect_hyperslab(globalspace, H5S_SELECT_SET, offset, stride, count, Nlocal);
  H5Pclose(plist);
  vector<int> buffer(Nglobal[0]*Nglobal[1]);
  ...
  int sum = 0;
  for (int i=0; i<Nglobal[0]*Nglobal[1]; i++)
    sum += buffer[i];
  printf("sum=%d\n",sum);
